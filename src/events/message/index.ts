...
const event = new Event('discord_client', 'message', async (client, message) => {
  ...
  const cmd = client.commands.get(comando) || client.commands.get(client.aliases.get(comando) || new String().normalize());

  if (!cmd) return;

  try {
    cmd.run({ message, args, client, comando, permissions, prefix });
  } catch (err) {
